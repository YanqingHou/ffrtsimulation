function usdt=runonce_cmpbench(num)
%% Initialization
tic
S=load('options.mat');
opts=S.opts;
...
savefilebench=strcat('./CmpARsNewlyFitMu2/CmpARs_',filename,'.txt');
...
for i=1:lenres
    ...
    tempfileffrt=strcat('./CmpARsNewlyFitMu2/CmpARsModel',num2str(num),'Epoch',num2str(i),'.txt');
...
parfor epi=1:epochsimulens
    ...
    i=epoch2simu(epi);
    tempfilebench=strcat('./CmpARsNewlyFitMu2/CmpARsModel',num2str(num),'Epoch',num2str(i),'.txt');
    [resbench]=bench_compare_OneEp(i,res(i).Qa,res(i).Qb,res(i).Qab,Nsamp,res(i).Ps,slicedPfixData{i,1},slicedTSRCData{i,1});
...
parfor i=1:16
    % for i=1:16
    slicefiles1{i,1}=strcat('./CmpARsNewlyFitMu2/CmpARsSliceModel',num2str(num),'Slice',num2str(i),'.txt');
    ...
    general_slicewrite(slicefiles1{i,1},'./CmpARsNewlyFitMu2/CmpARs','',num,epcs,printformatstr);
