function usdt=runonce_ffrt_falsealarm(num)
tic
S=load('options.mat');
opts=S.opts;
clear S;
...
savefileffrt=strcat('./resFFRTFalseAlarm/','FFRTFalseAlarm_',filename,'.txt');
% savefiletsrc=strcat('TSRC_',filename,'.txt');
% check if the epoch has been simulated
printformatstrs=cell(lenres,1);
epochstosimu=[];
for i=1:lenres
    tempfileffrt=strcat('./resFFRTFalseAlarm/','FFRTFalseAlarmModel',num2str(num),'Epoch',num2str(i),'.txt');
...
[resffrt]=ffrt_falsealarm_OneEp(i,res(i).Qa,Nsamp);
...
lenslice=lenres/16;
parfor i=1:16
    slicefiles1{i,1}=strcat('./resFFRTFalseAlarm/','FFRTFalseAlarmSliceModel',num2str(num),'Slice',num2str(i),'.txt');   
    epcs=lenslice*(i-1)+1:lenslice*i;
    general_slicewrite(slicefiles1{i,1},'./resFFRTFalseAlarm/FFRTFalseAlarm','',num,epcs,printformatstr);
end
