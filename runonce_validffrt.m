function usdt=runonce_validffrt(num)
tic
S=load('options.mat');
opts=S.opts;
clear S;
...
savefileffrt=strcat('./resValidFFRT/','ValidFFRT_',filename,'.txt');
% savefiletsrc=strcat('TSRC_',filename,'.txt');
% check if the epoch has been simulated
epochstosimu=[];
for i=1:lenres
    tempfileffrt=strcat('./resValidFFRT/','FFRTModel',num2str(num),'Epoch',num2str(i),'.txt');
...
parfor epi=1:epochsimulens
    ...
    i=epochstosimu(epi);
    tempfileffrt=strcat('./resValidFFRT/','FFRTModel',num2str(num),'Epoch',num2str(i),'.txt');
    [resffrt]=valid_fit_ffrt_OneEp(i,res(i).Qa,Nsamp);
...
lenslice=lenres/16;
parfor i=1:16
    slicefiles1{i,1}=strcat('./resValidFFRT/','FFRTSliceModel',num2str(num),'Slice',num2str(i),'.txt');
    epcs=lenslice*(i-1)+1:lenslice*i;
    general_slicewrite(slicefiles1{i,1},'./resValidFFRT/FFRT','',num,epcs,printformatstr);
end
