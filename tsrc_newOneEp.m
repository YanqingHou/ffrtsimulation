function [restsrc]=tsrc_newOneEp(ep,Qa,Nsamp,Psb,PfixData)%Qa,Qb,Qab,Ps, PfixData
%% input: Qa, Qb, Qab, Ps
...
for ns=minns2:na
    for ipf=1:Pfttlslen
        fixrate1{ns,ipf}=1-unfixnum1{ns,ipf}/Nsamp;
...
thefixrate1=zeros(na,Pf2slen);
ns1slen=0;
for ipf=1:Pfttlslen
    ns1slen=ns1slen+na-ns2(ipf);
end
restsrc=zeros(Pf2slen*ns1slen,13);
resrowcnt=0;
for ipf=1:Pfttlslen
    k2=na-ns2(ipf)+1;
...
%     for ns=ns2(ipf):na
    for ns=ns2(ipf)+1:na
        thefixrate1(ns,ipf)=fixrate1{ns,ipf}(1);
        for i=1:Pf2slen
            Pf2req=Pf2s(i);
        ...
        restsrc(resrowcnt,:)=[ep, na, Psb, Pfttls(ipf), ns, Pf1s(ipf), thefixrate1(ns,ipf), ns2(ipf), Pssub2, Pf2req, themu2(ns,i),thefixrate2(ns,i), thefailrate2(ns,i)];%Pfix1,
